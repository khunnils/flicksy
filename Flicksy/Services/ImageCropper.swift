...
enum ImageCropper {
    enum CropError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .unreadable: "This image could not be opened for cropping."
            case .invalidRect: "The crop area is too small."
            case .encodeFailed: "The cropped image could not be saved in this format."
    ...
    static func crop(url: URL, normalizedRect: CGRect) throws -> CGSize {
    ...
    private static func orientedFullImage(from source: CGImageSource) -> OrientedImage? {
    ...
    private static func pixelCropRect(normalized: CGRect, imageSize: CGSize) -> CGRect {
        let clamped = CGRect(
    ...
    private static func writeReplacing(url: URL, image: CGImage, preferredType: UTType?) throws {
        let type = writableType(preferred: preferredType)
        let temporary = FileManager.default.temporaryDirectory
            .appending(path: "flicksy-crop-\(UUID().uuidString).\(type.preferredFilenameExtension ?? "img")")
    ...
    private static func writableType(preferred: UTType?) -> UTType {
        if let preferred {
